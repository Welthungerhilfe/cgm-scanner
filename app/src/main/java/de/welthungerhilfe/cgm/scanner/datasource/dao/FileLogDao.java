...
public interface FileLogDao {
    ...
    @Query("SELECT * FROM " + TABLE_FILE_LOG + " WHERE scanServerId IS NOT NULL AND childHeightSynced=0 AND type='depth' AND environment=:environment ORDER BY createDate LIMIT 150")
    List<FileLog> loadAppHeightFileLog(int environment);

    @Query("SELECT * FROM " + TABLE_FILE_LOG + " WHERE scanServerId IS NOT NULL AND poseScoreSynced=0 AND type='rgb' AND environment=:environment ORDER BY createDate LIMIT 150")
    List<FileLog>  loadAppPoseScoreFileLog(int environment);

    @Query("SELECT * FROM " + TABLE_FILE_LOG + " WHERE scanServerId IS NOT NULL AND bounding_box_synced=0 AND type='rgb' AND environment=:environment ORDER BY createDate LIMIT 150")
    List<FileLog>  loadAppBoundingBox(int environment);

    @Query("SELECT * FROM " + TABLE_FILE_LOG + " WHERE scanServerId IS NOT NULL AND orientation_synced=0 AND type='depth' AND environment=:environment AND orientation IS NOT NULL ORDER BY createDate LIMIT 150")
    List<FileLog>  loadAppOrientation(int environment);

    @Query("SELECT * FROM " + TABLE_FILE_LOG + " WHERE scanServerId IS NOT NULL AND child_distance_synced=0 AND type='depth' AND environment=:environment ORDER BY createDate LIMIT 150")
