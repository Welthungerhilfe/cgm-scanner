...
public class ConsentScanActivity extends AppCompatActivity {
    ...
    private final ImageReader.OnImageAvailableListener mOnImageAvailableListener =
            new ImageReader.OnImageAvailableListener() {
                ...
                public void onImageAvailable(ImageReader reader) {
                    ...
                    } catch (ReaderException | NullPointerException ignored) {
    ...
    private final CameraDevice.StateCallback mStateCallback = new CameraDevice.StateCallback() {
        ...
        public void onError(CameraDevice cameraDevice, int error) {
            ...
            Toast.makeText(ConsentScanActivity.this, "Failed", Toast.LENGTH_LONG).show();
        }
    };
    ...
    private void setUpCameraOutputs(int width, int height) {
        ...
        try {
            for (String cameraId : manager.getCameraIdList()) {
                ...
                mImageReader = ImageReader.newInstance(largest.getWidth() / 20, largest.getHeight() / 20, sImageFormat, 2);
                mImageReader.setOnImageAvailableListener(mOnImageAvailableListener, new Handler());
    ...
    private void openCamera(int width, int height) {
        ...
        try {
            ...
            manager.openCamera(mCameraId, mStateCallback, new Handler());
        } catch (CameraAccessException e) {
            e.printStackTrace();
...
//                                mPreviewRequestBuilder.set(CONTROL_AE_MODE, CONTROL_AE_MODE_ON_AUTO_FLASH); // no need for flash now
                                ...
                                mCaptureSession.setRepeatingRequest(mPreviewRequest, mCaptureCallback, new Handler());
                            } catch (CameraAccessException e) {
                                e.printStackTrace();
                            }
                        }
                        ...
                        public void onConfigureFailed(CameraCaptureSession cameraCaptureSession) {
                            Toast.makeText(ConsentScanActivity.this, "Failed", Toast.LENGTH_SHORT).show();
                        }
                    }, null
            );
        } catch (CameraAccessException e) {
    ...
    public void onQRCodeRead(final String text) {
        txtAlert.setText(String.format("Qr code is readable - %s", text));
