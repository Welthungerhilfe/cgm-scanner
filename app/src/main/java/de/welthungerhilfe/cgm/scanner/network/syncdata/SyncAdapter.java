...
public class SyncAdapter extends AbstractThreadedSyncAdapter {
    ...
    private void postScans(HashMap<Integer, Scan> scans, Measure measure) {
        try {
            ...
            for (Scan scan : scans.values()) {
                ...
                retrofit.create(ApiService.class).postScans(session.getAuthToken(), body).subscribeOn(Schedulers.io())
    ...
    public void postPerson(Person person1) {
        try {
            ...
            retrofit.create(ApiService.class).postPerson(session.getAuthToken(), body).subscribeOn(Schedulers.io())
                    .observeOn(AndroidSchedulers.mainThread())
    ...
    public void putPerson(Person person1) {
        try {
            ...
            retrofit.create(ApiService.class).putPerson(session.getAuthToken(), body, person1.getServerId()).subscribeOn(Schedulers.io())
                    .observeOn(AndroidSchedulers.mainThread())
                    .subscribe(new Observer<Person>() {
                        @Override
                        public void onSubscribe(@NonNull Disposable d) {
    ...
    public void postMeasurment(Measure measure) {
        try {
            ...
            retrofit.create(ApiService.class).postMeasure(session.getAuthToken(), body).subscribeOn(Schedulers.io())
                    .observeOn(AndroidSchedulers.mainThread())
