...
public class Measure extends CsvExportableModel implements Serializable {
    ...
    public String getCsvFormattedString() {
        return String.format(Locale.US, "%s,%s,%d,%s,%d,%f,%f,%f,%f,%s,%b,%b,%d,%s,%b,%s,%s,%d,%b,%d,%d,%d,%f,%f,%s,%d,%s", id, personId, date, type, age, height, weight,
                ...
                heightConfidence, weightConfidence, scannedBy, environment, measure_updated);
    }

    @Override
    public String getCsvHeaderString() {
        return "id,personId,date,type,age,height,weight,muac,headCircumference,artifact,visible,oedema,timestamp,createdBy,deleted,deletedBy,qrCode,schema_version,artifact_synced,uploaded_at,resulted_at,received_at,heightConfidence,weightConfidence,scannedBy,environment,measure_updated";
    }
